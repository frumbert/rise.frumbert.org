...
function getSourceElement() {
    ...
    throw new Error(`No valid source element found in blockElement (${blockId})`);
}
...
async function initializePuzzle() {
    ...
    // Create a shared stream from the original video for memory efficiency and sync
    let sharedStream = null;
    if (tag === 'video') {
        try {
            const originalVideo = gget('source');
            originalVideo.autoplay = true;
            originalVideo.muted = true;
            originalVideo.loop = true;
            originalVideo.playsInline = true;
            originalVideo.play();
            sharedStream = originalVideo.captureStream();
        } catch (error) {
            console.warn('captureStream not supported, falling back to individual video sources');
        }
    }
    ...
    for (let row = 0; row < props.rows; row++) {
        for (let col = 0; col < props.cols; col++) {
            ...
            if (tag === 'video') {
                ...
                if (sharedStream) {
                    // Use shared stream - all videos will be synced and share memory
                    content.srcObject = sharedStream;
                } else {
...
function reSyncVideo() {
    // With shared stream, we only need to sync the original video
    // All puzzle pieces using the stream will automatically stay synced
    if (gget('source') && gget('source').tagName.toLowerCase() === 'video') {
        gget('source').currentTime = 0;
        gget('source').play();
    }
}
